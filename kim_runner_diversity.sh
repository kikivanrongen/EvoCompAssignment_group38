...
# do
#   echo $i
#   > diversitydata_parametertesting/cigar_as_ga_$i.txt
#   for j in {1..30}
#   do
#     java -Diter=$i -jar testrun.jar -submission=player38 -evaluation=BentCigarFunction -seed=1 >> diversitydata_parametertesting/cigar_as_ga_$i.txt
#   done
# done
#
...
do
  echo $i
  > diversitydata_parametertesting/katsuura_a1_1_$i.txt
  for j in {1..30}
  do
    java -Diter=$i -jar testrun.jar -submission=player38 -evaluation=KatsuuraEvaluation -seed=1 >> diversitydata_parametertesting/katsuura_a1_1_$i.txt
  done
done
