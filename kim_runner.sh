...
do
  ...
  for j in {1..30}
  do
    java -Diter=$i -jar testrun.jar -submission=player38 -evaluation=SchaffersEvaluation -seed=1 >> diversitydata/cigar_$i.txt
  done
done
...
do
  ...
  for j in {1..30}
  do
    java -Diter=$i -jar testrun.jar -submission=player38 -evaluation=KatsuuraEvaluation -seed=1 >> diversitydata/cigar_$i.txt
  done
done
