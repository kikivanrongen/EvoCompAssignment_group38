...
for i, item in enumerate(filenames):
    ...
    try:
        ...
        with open(item, 'r') as csvfile:
            ...
            if sys.argv[1] == 'cigar':
                data[0, int(item[7]), int(item[8])] = np.mean(score)
                data[1, int(item[7]), int(item[8])] = np.std(score)
...
sns.heatmap(data[0], annot=True, vmin=0, vmax=10, ax=ax, xticklabels=x_labels, yticklabels=y_labels)
plt.show()
